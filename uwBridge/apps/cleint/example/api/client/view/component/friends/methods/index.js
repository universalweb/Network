module.exports = async (app) => {
  ...
  const FriendScheme = thinky.models.friend;
  const friendMethodApi = {
    async accept(request) {
      ...
      const newTeam = new FriendScheme({
        to: body.item.from,
    ...
    async add(request) {
      ...
      if (user) {
        ...
        if (!relationshipCheck) {
          ...
          await friendMethodApi.create(request);
          response.data.status = 1;
        }
      }
    }
  };
  await api.extend(friendMethodApi, {
    prefix: 'friend'
  });
};
