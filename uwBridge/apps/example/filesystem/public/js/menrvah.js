...
/*
Platform: Menrvah BETA 1
Author: Thomas Marchi
Developer: Thomas Marchi
Designer: Thomas Marchi
Website: tommarchi.com - menrvah.com - aritysoftware.com
*/

$.menrvah = async function() {
  //localStorage.clear();
  ...
  cnsl('Spiral initialized', 'notify');

  cnsl('Menrvah Module', 'notify');
  const api = $.api = {},
    callbacks = {
      //when the worker is setup and the socket is ready
      async setupCompleted(data) {
        cnsl('Worker is Ready', 'notify');
        $.systemLanguage = data.language;
        try {
          await demand('menrvah/');
          await demand('app/');
        } catch (error) {
          console.log(error);
          localStorage.clear();
    ...
    mainWorker = new Worker('/js/menrvahWorker.js');
  api.request = apiRequest = async function(requestArg, dataArg) {
    let data;
    if (dataArg) {
      data = {
        request: requestArg,
    ...
    return promise((accept) => {
      const ogCallback = data.callback,
        uniq = uuid();
      callbacks[uniq] = (ogCallback) ? function(dataCallback) {
        accept(dataCallback);
        ogCallback(dataCallback);
      } : accept;
  ...
  mainWorker.onmessage = function(event) {
    ...
    callbacks[generatedId](data);
    if (!eventData.keep && generatedId !== '_') {
      callbacks[generatedId] = null;
      uuid.remove(generatedId);
    }
  };
  api.callbacks = callbacks;

  $.globalEvents = {};
  const modelCore = model.core,
    socketSuper = $.socket = {},
    loadedAbstract = $.loadedAssets = {},
    local = localStorage,
    headNode = querySelector('head'),
    styleNode = createTag('style'),
    commaString = ',',
    slashString = '/',
    isLibRegex = new RegExp(/^js\/lib\//);

  /*
  Async File requesting
  Requests are sent back

  Handle JSON/JS flat files then convert to iJSON
  Inline JSON is useful when using eval to
  return complex javascript objects or code to a variable or property.
  */
  const checksumReturn = (item) => {
      return local[`cs-${item}`];
    },
    constructStyleTagThenAppendToHead = (text, filepath) => {
    ...
    */
    getLoadedAssets = function(item) {
      return loadedAbstract[item];
    },
    getCompleted = async function(config) {
      const {
        callback,
        data
      } = config;
      const assetCollection = map(data, getLoadedAssets);
      callback.apply(callback, assetCollection);
    },
    checkIfCompleted = function(config) {
      if (!config.done && config.filesLoaded === config.fileCount) {
        config.done = true;
        getCompleted(config);
      }
    },
    saveCompleted = function(json, config) {
      const {
        file,
        cs,
        key,
      ...
      if (!hasValue(loadedAbstract[filename]) || fileContents !== true) {
        if (!isJs) {
          if (fileContents === false) {
            loadedAbstract[filename] = {};
          } else {
            loadedAbstract[filename] = (isJson) ? iJson(fileContents || {}) : fileContents;
          }
        } else if (fileContents) {
          if (isLibRegex.test(filename)) {
            new Function(fileContents)();
            loadedAbstract[filename] = top[modelName] || model[modelName];
          } else {
            const dirname = initial(filename.split(slashString))
              .join(slashString);
            if (loadedAbstract[filename]) {
              config.filesLoaded++;
              checkIfCompleted(config);
              return;
            }
            const exports = {
              _: {
            ...
            skipCheck = true;
            (async function() {
              await new Function('exports', `var {demand,component,app,request,pipe,push,notify,cnsl}=$; return ${fileContents}`)(exports);
              config.filesLoaded++;
              checkIfCompleted(config);
            })();
            loadedAbstract[filename] = exports;
          }
        }
      }
      if (isCss && appendCSS && isString(loadedAbstract[filename])) {
        constructStyleTagThenAppendToHead(loadedAbstract[filename], filename);
        loadedAbstract[filename] = true;
      }
      if (!skipCheck) {
        config.filesLoaded++;
    ...
    socketGet = $.fetchAsset = async function(config) {
    ...
    request = $.request = async function(name, configArg) {
      let config;
      if (!configArg) {
        config = name;
      } else {
      ...
      if (config.id) {
        data.data.data.id = data.data.id = config.id;
  ...
  const buildFilePath = (itemArg) => {
    ...
    singleDemand = function(items) {
      return items[0];
    },
    objectDemand = function(items, arrayToObjectMap) {
      return map(arrayToObjectMap, (item) => {
        return items[item];
      });
    },
    multiDemand = function(items) {
      return items;
    },
    streamAssets = async function(data, options) {
      return promise((accept) => {
        socketGet(assign({
          data,
          callback(...args) {
    ...
    demand = $.demand = async function(filesArg, options) {
      const assets = [];
      let demandType,
        arrayToObjectMap,
        files = filesArg;
    ...
    demandTypeMethod = function(type, optionsFunction) {
    ...
    demandCSS = $.demandCss = demandTypeMethod('css', (appendCSS) => {
      return {
        appendCSS
      };
    }),
    demandJs = $.demandJs = demandTypeMethod('js'),
    demandHtml = $.demandHtml = demandTypeMethod('html'),
    demandLang = $.demandLang = function(filesArg) {
      const files = (!isString(filesArg)) ? filesArg : filesArg.split(commaString);
      return demand(map(files, languageMethod));
    };
  ...
  $.isEventNode = (event) => {
    if (!event || !event.original || !event.original.target) {
      return false;
    }
    return event.node === event.original.target;
  };

  const isLang = new RegExp(/^language\//),
    languageMethod = $.language = function(stringArg) {
      let string = stringArg;
      if (!isLang.test(string)) {
        if (first(string) !== '/') {
          string = `/${string}`;
        }
        string = `language${string}`;
      }
      string = (last(string) === '/') ? `${string}${$.systemLanguage}.json` : `${string}/${$.systemLanguage}.json`;
      return string;
    };

  $.super((key, data) => {
    return (data && data.invoke ? (hasAnyKeys(data, socketImportProperties) ? proxyModule : proxyDefine) : model)(key, data);
  });

  cnsl('Pipes setup', 'notify');
  const updateSubs = {},
    updateSubsRegex = [],
    onUpdate = async function(json) {
      if (!pipe.status) {
        return;
      }
      const type = json.type;
      const subscribers = [];
      const levelObject = updateSubs[type];
      const regexUpdates = filter(updateSubsRegex, (item) => {
        if (item.regex.test(type)) {
          return item;
        }
      });
      if (!isEmpty(regexUpdates)) {
        subscribers.push(...regexUpdates);
      }
      if (levelObject) {
    ...
    pipeOnCollection = (object, settings) => {
      const pipes = [],
      ...
      each(object, (item, key) => {
        pipes.push(pipeOn(`${prefix}${key}${suffix}`, item, settings));
      });
      return pipeCollection;
    },
    pipeOnRegex = (type, streamArg, options) => {
      const pipeObject = {};
      const stream = streamArg.bind(options || pipeObject);
      stream.regex = type;
      let number = updateSubsRegex.push(stream) - 1;
      assign(pipeObject, {
        stop() {
          if (hasValue(number)) {
            drop(updateSubsRegex, number);
            number = null;
          }
        },
        start() {
          if (!hasValue(number)) {
            number = updateSubsRegex.push(stream) - 1;
          }
        },
        stream,
        _: {
    ...
    pipeOnString = (type, streamArg, options) => {
      const pipeObject = {};
      const stream = async function(...args) {
        if (options && options.security && await options.security()) {
          return;
        }
        streamArg.call(options || pipeObject, ...args);
      };
      if (!updateSubs[type]) {
        updateSubs[type] = [];
      }
      const levelObject = updateSubs[type];
      let number = levelObject.push(stream) - 1;
      assign(pipeObject, {
        stop() {
          if (hasValue(number)) {
        ...
        start() {
          if (!hasValue(number)) {
            number = levelObject.push(stream) - 1;
          }
        },
        stream,
        _: {
    ...
    pipeOn = pipe.on = (type, funct, options) => (isString(type) ? pipeOnString : (isPlainObject(type)) ? pipeOnCollection : pipeOnRegex)(type, funct, options),
    pipePush = $.push = (requestName, data) => request({
      id: '_',
      request: requestName,
      data,
    });
  pipe.watchers = updateSubs;
  pipe.status = true;
  api.callbacks._ = async function(json) {
    onUpdate(json.data);
  };
  $.pipeUpdates = updateSubs;
