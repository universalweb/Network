...
export async function importKMACKey(key, keyLength) {
	...
	if (isBuffer(key) || isU8(key)) {
		return subtle.importKey('raw-secret', key, {
			name: 'KMAC256',
			length: ((keyLength || key.byteLength || key.length) * 8),
		}, true, ['sign', 'verify']);
	}
	return key;
}
...
export async function kmac256(message, key, outputLength = 32, customization) {
	if (useAddon) {
		return kmac256JS(key, message, outputLength, {
...
export async function kmac256Verify(signature, message, key, outputLength = 32, customization) {
	if (useAddon) {
		const sig2 = kmac256JS(key, message, outputLength, {
			personalization: customization,
		});
...
async function exampleKMAC() {
	...
	const sig1 = await kmac256(message, key, 32, Buffer.from('MyApp'));
	console.log('KMAC256 sig (hex):', Buffer.from(sig1).toString('hex'));
	const sig2 = await kmac256(message, key, 32, Buffer.from('MyApp'));
	console.log('KMAC256 sig2 (hex):', (kmac256JS(rawKey, message, 32, {
	...
	const valid = await kmac256Verify(sig1, message, key, 32, Buffer.from('MyApp'));
