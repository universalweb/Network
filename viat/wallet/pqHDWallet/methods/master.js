...
export async function generateMasterNonce(size, structure = {}) {
	this.info.masterNonce = await this.generateMasterNonce(size, structure);
}
export async function generateMasterKey(size, structure = {}) {
	this.info.masterKey = await this.generateMasterKey(size, structure);
}
export async function generateMasterSeed(config = {}, source = {}) {
	...
	console.log('generateMasterSeed', source);
	...
	console.log('generateMasterSeed encode', this.info.masterSeed);
...
export async function generateMasterSeedStruct(config, source = {}) {
	...
	source.seed = await this.generateMaster();
