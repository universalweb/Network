/*
 * Wallet app entry — boot splash FIRST, then dynamic-load the rest.
 *
 * The document at / (client/index.html) points here. Shared framework
 * code stays under modules/ and components/; this file is the wallet's
 * pair with index.css. Static imports are intentionally lean. config.js
 * must be first (core/debug/logger.js reads globalThis.CONFIG at module
 * init). theme-manager is the other required static: it owns the theme
 * <link> and its top-level boot await makes the chosen sheet live before
 * the splash mounts. That is one extra CSS fetch before first paint —
 * the intended trade so the page never paints unthemed. BootScreen +
 * BootPipeline are the splash. Everything else is dynamic-imported
 * inside the boot pipeline after the splash is live
 * (see modules/boot-pipeline.js).
 */
import '../../modules/config.js';
/* Owns and creates the theme <link>; its top-level boot await means the
   sheet is live before the splash mounts. Imported explicitly rather than
   leaned on through the dynamic app graph. */
import '../../components/global/theme-select/theme-manager.js';
