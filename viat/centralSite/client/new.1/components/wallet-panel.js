...
export class WalletPanel extends WebComponent {
	...
	set walletAddress(v) { this.updateState({ walletAddress: v ?? '' }); }
	get received() { return this.state.received; }
	set received(v) { this.updateState({ received: v ?? '0' }); }
	get sent() { return this.state.sent; }
	set sent(v) { this.updateState({ sent: v ?? '0' }); }
	get activity() { return this.state.activity; }
	set activity(v) { this.updateState({ activity: v ?? '0' }); }
	render() {
		this.html`
			<div class="panel">
