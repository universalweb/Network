...
**Intersection and visibility are NOT in the phase ladder.** They are observer-driven viewport-state signals, not structural lifecycle milestones — exposed as standalone properties (`isIntersecting`, `isIntersected`, `isVisible`) and Promises (`whenIntersected`, `whenVisible`). This separation keeps the phase ladder a pure structural-progression ratchet (about DOM presence, render state, mount state) while letting viewport observation toggle independently.
...
| `onIntersect(isIntersecting)` | `onLive` resolved (observer installed only after `onLive`); V1 or V2 IO threshold crossed; `isConnected === true`. **No phase advance** — updates `isIntersecting` / `isIntersected` and resolves `whenIntersected` (first fire only) |
...
For "all children ready" — covered by self's `onMount` (the bottom-up await chain guarantees it). For "react to each child as it mounts" — children explicitly notify parent via direct method call inside their own `onMount`:

```js
// Child opts in to notifying parent
class Item extends WebComponent {
  async onMount() {
    this.parentComponent?.handleChildMounted?.(this)
  }
}
```

This is cheaper than event listener machinery and makes the relationship explicit at the child's call site.

#### The dock/active-bar pattern

```js
class ActiveBar extends WebComponent {
  onLive() {
    // Self + ancestors + siblings all have settled layout (bottom-up await chain
    // + document-wide layout pass). Measure freely.
    const parent = this.parentComponent
    const active = parent.findComponent('dock-item', i => i.state.active)
    const rect = active.getBoundingClientRect()
    this.style.transform = `translateX(${rect.left}px)`
  }
  onVisible() {
    // Self is in viewport; parent is too (transitively). Start visible-only work.
    this.classList.add('animating')
  }
}
```

`onLive` for layout-dependent measurement; `onVisible` for viewport-dependent activation. **No parent hooks needed.**

#### Patterns that explicitly need cross-component coordination

```js
// Wait for parent's setup to complete (parent does work in its onMount)
async onMount() {
  await this.parentComponent.whenMounted
...
async onMount() {
  await Promise.all(
    this.getComponentsArray('list-item').map(c => c.whenLive)
  )
  this.calibrateLayout()
}

// Track each child mounting (child notifies parent explicitly)
class Container extends WebComponent {
  handleChildMounted(child) {
    this.activeChildren.add(child)
  }
}
class ContainerChild extends WebComponent {
  async onMount() {
    this.parentComponent?.handleChildMounted?.(this)
  }
}
...
All achievable with the three primitives. No events, no extra hooks.
...
| `whenIntersected` | `onIntersect` fires (V1 IO) | start of disconnect |
| `whenVisible` | `onVisible` fires (V2 IO) | start of disconnect |
| `whenTreeIntersected` | self.`whenIntersected` resolves AND every direct child's `whenTreeIntersected` resolves (recursive) | start of disconnect |
...
`onIntersect` and `onVisible` fire independently per component (driven by their respective IntersectionObservers). The tree-aware Promises compose recursively: a parent's `whenTreeVisible` resolves only when its own `whenVisible` AND every direct child's `whenTreeVisible` resolves. Same pattern for `whenTreeIntersected`.

This gives a bottom-up "entire subtree is visible/intersected" signal without making the per-component hooks themselves bottom-up — which would risk stalling parent forever if a child is hidden via `display:none` or scrolled out.
...
`whenTreeIntersected` and `whenTreeVisible` exist because observer hooks are independent per component (no bottom-up compose) — `parent.whenVisible` resolving doesn't say anything about its children. The tree Promise is the only way to express "subtree is visible" for those signals.

Rule: **structural Promises (`whenMounted`, `whenRendered`, `whenLive`) are already tree-aware via the await chain. Observer Promises (`whenIntersected`, `whenVisible`) need explicit tree variants.**
...
class MyComp extends WebComponent {
  config = {
    intersectThrottle: 250,   // V2 between-callback throttle (default 100)
  }
}
```

Defaults live as a base-class field initializer; subclasses override by redefining the `config` initializer. No deep merge across the prototype chain — the nearest class's initializer wins (same nearest-override pattern as `static intersectThreshold`). To extend rather than replace, spread explicitly:

```js
class MyComp extends WebComponent {
  config = { ...new WebComponent().config, intersectThrottle: 250 }
  // (or copy the small handful of base defaults inline)
}
```

In practice the `config` object is small enough that flat re-declaration is preferred over spread.
...
This is deliberate. Most config keys map onto primitives that can't be retuned without being rebuilt. Treat `config` as initial conditions, not live state.

### Lock contract

`config` is a plain object in v1 — not frozen, not proxied. The framework reserves the right to `Object.freeze(this.config)` after the last consumption point (today: observer install), turning post-consumption mutations into loud errors in strict mode. Not enabled by default in v1 to keep ergonomics open; opt-in flag (`static lockConfig = true`) is reserved for once the consumption surface stabilizes.
...
Existing framework names being renamed:

| Old | New |
|---|---|
| `mounted` (Promise) | `whenMounted` |
| `renderComplete` (Promise) | `whenRendered` |
| `onMounted` (hook) | `onMount` |
| `onRenderComplete` (hook) | folded into `onRendered` |
...
The framework runs **internal handlers** for every phase. The handler does the framework-side work (advance phase, resolve Promises, update flags, dispatch error to error hooks) and *then* invokes the user's hook if defined. This pattern matches the existing `handleConnectedCallback` / `handleDisconnectedCallback` in [base.js](base.js) and extends to every spine event.

```js
// Conceptual framework-internal handler for the mount transition
async handleMount() {
  this.phase = 'mounted'
  this.whenMountedResolver()
  if (this.onMount) {
    try {
      await this.onMount()
    } catch (error) {
...
1. **Framework state always advances correctly**, even if the user defines no hook. Phase moves, Promises resolve, observation flags toggle — all driven by handlers, not by user code.
...
|---|---|
| First `entry.isIntersecting === true` | `isIntersecting = true`, `isIntersected = true`, `whenIntersected` resolves, `onIntersect(true)` fires |
...
| `onLive` | stays `mounted` | `whenLive` resolves | `onRenderError` |
| `onIntersect` | n/a (no phase advance) | `whenIntersected` resolves anyway (if first fire); state flags updated regardless of throw | `onLifecycleError` |
...
|---|---|
| `onConnect` | finish (awaited by `connectPromise`); pipeline doesn't proceed |
...
| `onDisconnect` running | runs to completion via `connectPromise` await |
...
- `templateBuilt = false`
- All Promises (`whenConnected`, `whenMounted`, `whenLive`, `whenIntersected`, `whenVisible`, `whenTreeIntersected`, `whenTreeVisible`) recreated as new pending
...
constructor:
  ...
  whenIntersected      = new pending
  whenVisible          = new pending
  whenTreeIntersected  = new pending (lazy-computed at first access)
...
connect:
  ...
  whenIntersected resolves after first onIntersect fire (regardless of direction)
  whenVisible resolves after onVisible (one-shot per cycle)
  whenTreeIntersected resolves after self.whenIntersected AND every direct child's whenTreeIntersected
...
disconnect:
  ...
  whenConnected, whenMounted, whenLive, whenIntersected, whenVisible recreated as new pending
  whenTreeIntersected, whenTreeVisible recreated as new pending
...
- If `isIntersecting === true`: bounds are in viewport at configured threshold; `isIntersecting` is now `true`; if first fire, `isIntersected` is now `true` and `whenIntersected` has resolved
...
// 5. Gate child on parent's setup
...
if (comp.atPhase('visible')) startEntryAnimation()

// 8. Wait for all current children to mount
await Promise.all(this.getComponentsArray('list-item').map(c => c.whenMounted))

// 9. Child explicitly notifies parent at mount
class Item extends WebComponent {
  async onMount() {
    this.parentComponent?.handleChildMounted?.(this)
  }
...
These are explicitly **not in v1** but worth keeping in mind. Each can be revisited if a real use case appears.

### DOM event dispatch (deferred)

The framework currently dispatches no lifecycle events. If we ever add them, they must NOT be implemented as standard `dispatchEvent` + `addEventListener` calls — that's heavy and creates listener-attachment timing problems (a listener added in `onConnect` already missed any earlier event).

If added, they would be a custom internal event system optimized for the lifecycle case:
- A small per-component listener map (not the full DOM event flow)
- Listeners registered via `comp.onPhase('mount', cb)` with explicit cleanup tied to the listener's owning component's lifecycle
- Composed traversal across shadow boundaries done by the framework, not via `composed: true` event bubbling

Even then, this layer should be opt-in. The default lifecycle stays pure function calls and Promises.

### `onParentMount` / `onChildMount` (deferred)

Useful only if a real pattern emerges that's awkward with `await parentComponent.whenMounted` or `await Promise.all(children.map(c => c.whenMounted))`. Flag them as candidates if the manual-Promise pattern shows up repeatedly in real code.

If added, they would be direct method calls (not events): the parent's own transition triggers the framework to call each registered child's `onParentMount(parent)` directly, in registration order.

### Adopted callback (`onAdopt`)

Skipped because moving elements between documents is vanishingly rare in real apps. If a use case appears, add `onAdopt(oldDoc, newDoc)` parallel to `onMove`.

## Comparison to Current Codebase

The spec is an evolution of the existing `base.js` approach, not a replacement of its philosophy. The current code already uses function calls + Promise resolvers (no events); the spec tightens semantics, adds missing phases, and consolidates state.

### What stays architecturally

| Today's pattern | Spec preserves |
|---|---|
| Function calls + `await` for hooks (no events) | identical |
| `handleConnectedCallback` / `handleDisconnectedCallback` internal-handler pattern | extended to every phase (`handleMount`, `handleLive`, `handleVisibilityCallback`, etc.) — handlers do framework state work and invoke user hooks if defined |
| Promise resolvers stored on instance (`mountedResolver`, `renderResolver`) | same pattern, more of them |
| Sequence-based render cancellation (`renderSeq`) | kept |
| `isConnected` check between async hooks | formalized as transition precondition |
| `connectPromise` to await pending connect during disconnect | kept |
| `register` / `registerChild` for parent-child tracking | kept |
| `waitForRenderedTree` recursive subtree await | kept (used by per-phase awaits) |
| `IntersectionObserver` for `onVisible` | split semantically: `onIntersect` (bidirectional, V1 IO or shared V2) and `onVisible` (one-shot, V2 IO with V1+manual-check fallback). Single observer per component (shared V2 when both hooks overridden). Standalone state flags (`isIntersecting`, `isIntersected`, `isVisible`) replace phase-tracked visibility |
| `'mounting'` class for first-render UX | kept |
| Per-render proxy and `setCurrentTracking` | kept |
| `cleanupTemplate`, subscription clearing in disconnect | kept |

### What's new

| Spec addition | What it adds |
|---|---|
| `onInit(state, config)` | Template method called inside super constructor for subclass setup before reactivity arms |
| `onLive()` | Splits out of current `onMounted` — post-rAF guarantee separated from "subtree settled" |
| `phase` property | Consolidates `firstRenderDone`, `templateBuilt`, `visibleFired` into one ordinal value |
| `is*` getters | Derived from `phase` — replaces ad-hoc internal flag checks |
| `whenLive`, `whenIntersected`, `whenVisible`, `whenTreeIntersected`, `whenTreeVisible`, `whenConnected`, `whenDisconnected`, `whenDestroyed` Promises | Today only `mounted` and `renderComplete` exist |
| `onIntersect` hook (bidirectional) | Today's `onVisible` is V1 IO one-shot; spec adds bidirectional `onIntersect(isIntersecting)` for continuous tracking, plus a new one-shot `onVisible` (V2 trackVisibility) for true user-visibility |
| Standalone observation state (`isIntersecting`, `isIntersected`, `isVisible`) | Today there's no current-state intersection tracking — only the `visibleFired` flag |
| `nextFrame()` batched scheduler | Today there are 3 separate `requestAnimationFrame` calls scattered (`preRender`, `waitForRenderedTree`, mounting class removal). Spec consolidates to one batched queue |
| `parentComponent` accessor | Today the parent host is computed inline in `handleConnectedCallback`. Spec promotes it to a stable cached accessor |
| Explicit transition preconditions (`isConnected` re-checked at every step) | Today this is implicit; spec formalizes |
...
|---|---|
| `onMounted()` | `onMount()` |
| `onRenderComplete()` | folded into `onRendered()` |
| `onRendered()` (fires only on subsequent renders) | fires on every render including first |
| `mounted` Promise | `whenMounted` |
| `renderComplete` Promise | `whenRendered` |
...
| Today | Why removed |
|---|---|
| `usesMountLifecycle()` predicate | Mount path always runs; cost of always-running is negligible; logic was complicating the code |
| `onRenderComplete` as separate hook | Redundant with `onRendered` once `onRendered` fires every render |
| Scattered `requestAnimationFrame` calls | Replaced by single batched `nextFrame()` |
| `firstRenderDone`, `visibleFired`, `templateBuilt` as separate public fields | Consolidated into `phase` (these become internal implementation details, not public state) |
| Stub method bodies on the prototype (e.g., `onVisible() {}`, `onMounted() {}`) | All hooks default to `undefined`; truthy-check replaces prototype comparison for override detection |

### What was considered but NOT added (see Future Considerations)

| Considered | Decision |
|---|---|
| DOM events `mount`, `live`, `visible`, `disconnect` | Deferred — if added, must be a lightweight internal listener system, not full DOM event flow |
| `onParentMount`, `onChildMount`, etc. | Deferred — direct method calls, not events; revisit if patterns emerge |
| `onAdopt` for `adoptedCallback` | Skipped — vanishingly rare |
| `awaitPhase(target)` method | Skipped — direct `whenX` properties cover the need |
| `componentAncestors` generator | Skipped — recursive `parentComponent` access works |
...
5. Ten Promise contracts: `whenConnected`, `whenRendered`, `whenMounted`, `whenLive`, `whenIntersected`, `whenVisible`, `whenTreeIntersected`, `whenTreeVisible`, `whenDisconnected`, `whenDestroyed`
...
8. Cross-component ordering enforced via per-phase Promise chains (bottom-up): `onRendered` → child `whenRendered`, `onMount` → child `whenMounted`, `onLive` → child `whenLive`. `whenTreeVisible` is recursive: self.`whenVisible` AND every direct child's `whenTreeVisible`
...
14. **No dedicated cross-component hooks in v1.** Coordination via Promises, `parentComponent` accessor, and explicit child→parent method calls. `onParentMount` / `onChildMount` deferred to future consideration
...
21. **All `on*` hooks are undefined by default on `WebComponent.prototype`** — no stub method bodies. Detection inside the framework is a truthy check (`if (this.onMount)`), not a prototype-comparison. Internal `handleX` handlers always run (advance phase, resolve Promises, update flags); they call user-defined hooks if present and route hook errors through `onLifecycleError` / `onRenderError` (also undefined-by-default). This pattern matches the existing `handleConnectedCallback` / `handleDisconnectedCallback` style in `base.js` and extends to every spine event
...
23. Per-instance tunables live under a single `config` object on the component (e.g., `config.intersectThrottle`); per-class tunables stay as `static` fields (e.g., `static intersectThreshold`). `config` defaults live in the base-class field initializer; subclass override is by re-declaration (no deep merge). Framework reads each key **once** at consumption time (e.g., observer install); later mutations are silent no-ops because the value has already been baked into the underlying primitive. `config` is a plain unfrozen object in v1 — `Object.freeze` lock-on-init reserved as a future opt-in (`static lockConfig = true`) once the consumption surface stabilizes. No reactivity layer; runtime retuning, when needed, will go through targeted methods that rebuild the affected primitive
...
- `whenTreeIntersected` / `whenTreeVisible` implementation: getters that lazily compute the recursive Promise (`Promise.all([this.whenVisible, ...allChildren(this).map(c => c.whenTreeVisible)])`) at first access; cache the resulting Promise per connect cycle; recreate at disconnect
...
- Observer callback (`handleObserverCallback`): on every callback,
  ...
  - if first true-intersect: `this.isIntersected = true`, resolve `whenIntersected`
...
- Reset on disconnect: `isIntersecting = false`, `isIntersected = false`, `isVisible = false`, `visibleFired = false`. Disconnect observer. Recreate `whenIntersected` and `whenVisible` Promises
...
- `destroy()` implementation: set an internal `pendingDestroy` flag, call `this.remove()` (triggers `disconnectedCallback` → `onDisconnect`); after `onDisconnect` resolves, check the flag, run `onDestroy`, set `phase = 'destroyed'`, resolve `whenDestroyed`. The flag distinguishes destroy-driven disconnect from normal disconnect
