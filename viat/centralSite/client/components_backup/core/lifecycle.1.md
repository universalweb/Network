...
| `connectPromise` to await pending connect during disconnect | kept (renamed to `pendingConnect` for naming clarity vs. public `whenConnected`) |
...
| `whenLive`, `whenVisible`, `whenTreeVisible`, `whenConnected`, `whenDisconnected`, `whenDestroyed` Promises | Today only `mounted` and `renderComplete` exist |
...
- The internal "wait for in-flight connect to finish before disconnect tears down" Promise (currently `connectPromise` in [base.js](base.js)) is **renamed to `pendingConnect`** to avoid confusion with the public `whenConnected` Promise. They are different signals: `whenConnected` resolves when `onConnect` itself resolves (the structural milestone); `pendingConnect` is an internal field holding the full connect-chain Promise, awaited by `handleDisconnect` so subscriptions and observers aren't torn down mid-render. `pendingConnect` is internal-only; user code never sees it
