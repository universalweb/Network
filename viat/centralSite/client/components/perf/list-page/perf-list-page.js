...
import { PerfDeepNode, PropagationTracker } from '../deep-node/perf-deep-node.js';
import { PerfFullCard } from '../full-card/perf-full-card.js';
import { PerfListItem } from '../list-item/perf-list-item.js';
...
export class PerfListPage extends WebComponent {
	...
	render() {
		...
		const reportTableState = {
			heading: 'Operation stats — last benchmark',
		...
		const censusTableState = {
			...
			items: censusToRows(this.state.census),
			emptyMessage: '—',
		};
		const tagTableState = {
			heading: 'Top tags (live)',
			columns: TAG_COLUMNS,
			items: tagBreakdownRows(this.state.census),
			emptyMessage: '—',
		};
		const memoryTableState = {
			...
			items: memoryRows(this.state.memory),
			emptyMessage: '—',
		};
		const archTableState = {
			heading: 'Framework architecture (subsystem map)',
			...
			items: ARCHITECTURE_ROWS,
		};
		const depthTableState = {
			heading: 'Propagation timing (last run)',
