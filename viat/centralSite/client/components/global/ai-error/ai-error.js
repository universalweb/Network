...
export class UIAiError extends WebComponent {
	...
	render() {
		this.html`
			<section class="ai-error" data-kind=${this.state.kind} ?hidden=${!this.showError} role="alert">
				<header class="ai-error-head">
					<ui-icon class="ai-error-icon" .state.name=${this.kindView.icon} .state.size=${'sm'}></ui-icon>
					<span class="ai-error-title">${this.kindView.heading}</span>
					<button type="button"
						class="ai-error-dismiss"
				...
				<p class="ai-error-message">${this.state.message}</p>
				<footer class="ai-error-actions" ?hidden=${!this.state.retryable}>
