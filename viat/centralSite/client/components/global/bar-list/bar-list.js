...
export class UIBarList extends WebComponent {
	...
	barRow(item) {
		...
		return item && item.href ? html`<li class="bar-list-row"><div class="bar-list-track"><span class="bar-list-bar" style=${`inline-size:${pct}%`}></span><a class="bar-list-label bar-list-link" href=${item.href}>${item?.label}</a></div><span class="bar-list-value">${value}</span></li>` : html`<li class="bar-list-row"><div class="bar-list-track"><span class="bar-list-bar" style=${`inline-size:${pct}%`}></span><span class="bar-list-label">${item?.label}</span></div><span class="bar-list-value">${value}</span></li>`;
	}
	render() {
		this.html`
			<ol class="bar-list" data-tone=${this.toneClass}>
