...
// object. Same EventListener-object pattern as EventEntry: the wrapper IS the
// listener (its `handleEvent` is this function), so the browser calls
// `wrapper.handleEvent(domEvent)` with `this = wrapper`. Routes through the
// component's own `runEventHandler` so async errors land in the standard
// `handleEventError` path.
...
// `this.addEvent(eventName, handler, element?, options?)` — the unified
// primitive. Element defaults to the component itself. Returns an `EventEntry`
// with `.unsubscribe()`. The entry is auto-tracked in `component.eventEntries`
// and swept on disconnect via `clearEventListeners`. Both positional and
// object-form invocation are accepted.
...
// `this.listener(handler)` — escape hatch for raw `addEventListener` wiring on
// an element you do not want auto-tracked. Returns a cached EventListener-
// object (one per `(component, handler)` pair) that the browser invokes via
// its `handleEvent`. WeakRef on component so a forgotten registration cannot
// pin its component alive.
...
// `this.on(name, handler, options?)` — thin wrapper over `addEvent` that
// pins the element to the component itself. Returns the same `EventEntry`
// shape addEvent returns, so callers can hold the entry and call
// `entry.unsubscribe()`. Validation lives in `addEvent`.
export function on(eventName, handlerFunction, options) {
	return this.addEvent(eventName, handlerFunction, this, options);
}
// `this.once(name, handler, options?)` — merge `once: true` into options
// and route through `on()`. `EventEntry.handleEvent` honors `fireOnce`.
...
// `this.off(name, handler?, options?)` — criteria-match `eventEntries` and
// unsubscribe each match. Constrained to entries whose element IS the
// component, so `off()` cannot reach beyond the `on()` contract; entries
// from `addEvent(..., someOtherElement)` are handled by holding the entry
// reference and calling `entry.unsubscribe()` directly.
