...
export class Subscription {
	constructor(bus, path, handler, target) {
		this.bus = bus;
		this.path = path;
...
export class PathSubscriptions {
	 ...
	 */
	onFlush() {}
	subscribe(path, handler, target) {
		return new Subscription(this, path, handler, target);
	}
	notify(path) {
	...
	flush() {
		...
		if (this.subs.size) {
			...
			for (let i = 0; i < entries.length; i++) {
				...
				// overlapping changed path. The `break` after dispatch
				// enforces "at most once."
				for (let j = 0; j < changed.length; j++) {
					...
					for (let k = 0; k < subscriptionArray.length; k++) {
						fireSubscription(subscriptionArray[k], value, changedPath);
					}
					break;
				}
			}
