...
export class UISkeleton extends WebComponent {
	...
	get hostClass() {
		return `skeleton variant-${this.state.variant}`;
	}
	renderLines() {
		const out = [];
	...
	render() {
		...
		this.html `
			<div class="${() => this.hostClass}" aria-busy="true" aria-live="polite">
				${() => (this.isMulti ? this.renderLines() : `<span class="skeleton-line" style="${this.singleStyle}"></span>`)}
			</div>
		`;
	}
}
