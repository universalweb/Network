...
class UIPullDown extends WebComponent {
  onMount() {
    ...
    // No onUnmount needed. Auto-cleaned on disconnect.
  }
...
| Auto-cleaned on `disconnectedCallback` via `delegateUnsubs`                             | You must remember to remove it in `onUnmount`                                    |
...
| `this.refsMap`        | raw `Object.create(null)` of `WeakRef`s | advanced — usually leave alone |
...
### Memory contract
Storage is `Object.create(null)` with `WeakRef<Element>` values plus a `FinalizationRegistry` safety net.
