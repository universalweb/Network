import {
	isFunction, isObject, isPromiseLike, isString,
} from './utilities.js';
...
export function delegate(channel, handler, options) {
	...
	let subMap = channels.get(eventName);
	if (!subMap) {
		subMap = new Map();
		channels.set(eventName, subMap);
	}
	let handlers = subMap.get(subeventName);
	if (!handlers) {
		handlers = new Set();
		subMap.set(subeventName, handlers);
	}
	const entry = {
		handler,
		wrapped: null,
...
export function registerSubevent(element, subeventName, data) {
	let map = subevents.get(element);
	if (!map) {
		map = new Map();
		subevents.set(element, map);
	}
	map.set(subeventName, data);
}
