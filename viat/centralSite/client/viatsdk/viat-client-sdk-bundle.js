...
import { keys, noValue, isPlainObject, hasValue, isBuffer as isBuffer$1, isU8, isUndefined, isFunction, isString, isNumber, isBoolean, isBigInt, isMap, eachObject, isArray, eachArray, extendClass, assign } from '@universalweb/utilitylib';
...
function bytesToNumberLE(bytes) {
    return hexToNumber$1(bytesToHex$1(copyBytes$1(abytes$1(bytes)).reverse()));
}
/**
...
function edwards(params, extraOpts = {}) {
    ...
    class Point {
        ...
        static fromBytes(bytes, zip215 = false) {
            const len = Fp.BYTES;
            const { a, d } = CURVE;
            bytes = copyBytes$1(abytes(bytes, len, 'point'));
            abool(zip215, 'zip215');
            const normed = copyBytes$1(bytes); // copy again, we'll manipulate it
...
function montgomery(curveDef) {
    ...
    function decodeU(u) {
        const _u = copyBytes$1(abytes(u, fieldLen, 'uCoordinate'));
    ...
    function decodeScalar(scalar) {
        return bytesToNumberLE(adjustScalarBytes(copyBytes$1(abytes(scalar, fieldLen, 'scalar'))));
    }
...
class _RistrettoPoint extends PrimeEdwardsPoint {
    ...
    static fromBytes(bytes) {
        ...
        if (!equalBytes$1(Fp.toBytes(s), bytes) || isNegativeLE(s, P))
...
function gen(opts, hashOpts_) {
    ...
    const internal = Object.freeze({
        ...
        keygen(seed) {
            ...
            seed = seed === undefined ? randomBytes$1(seedCoder.bytesLen) : copyBytes(seed);
...
function getIvLength(options = {}, meta = undefined) {
	return options?.ivLength || meta?.cipher?.ivLength || 12;
}
function toAesKeyBytes(source) {
...
export { VIATClientSDK, decode, VIATClientSDK as default, encode };
