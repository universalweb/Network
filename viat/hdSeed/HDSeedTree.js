...
async function example() {
	const trieExample = new HDSeedTree({});
	await trieExample.createRoot();
	console.log('Root Seed:', await trieExample.getRootSeed());
	const branchViatWallet = await trieExample.root.addBranch('wallet', {
		path: 'crypto.viat.wallet',
		id: 1,
		horizontal_id: 0,
	});
	const branchViatWalletSeed = await trieExample.root.getBranch('wallet');
