...
The system is fundamentally: multi-source → multiple entropy pools → structured metadata binding via CBOR → independent parallel derivation paths → combination through KMAC → scheme-specific output seed.
...
KMAC(KEY, FINAL SEED, CUSTOMIZATION)

- K [HASH] (KEY) = derived from master_key (secret entropy)
- M [HASH] (MESSAGE, FINAL SEED, DATA) = derived from master_seed
- S [STRUCTURE] (CUSTOMIZATION, PERSONALIZATION) = derived from master_nonce

K and M are entropy, S is structure — The key carries secret material from master_key. The message carries secret material from master_seed. The customization string is the only KMAC input that still contains readable, structured, deterministic context.

Collision resistance at the KMAC level — Two derivations with identical key + final-seed (MESSAGE) but different metadata MUST produce different outputs. The customization string is exactly what ensures this.

The nonce is in S — The nonce is derived from master_nonce via its own parallel path, so it's an independent entropy contribution. Placing the derived nonce in the customization string alongside the metadata means S carries both the structural context AND a third independent entropy pool.
